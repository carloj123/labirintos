...
lista = list()
def escreveLinha(x1,y1,x2,y2,arquivo):
        arquivo.write("\t\t\t<polyline points=\"%d,%d %d,%d\" />\n" % (x1,y1,x2,y2))

def escreveCirculo(x, y,arquivo,metragem):
    metragem = metragem
    arquivo.write("\t\t\t<circle cx=\"%.1f\" cy=\"%.1f\" r=\"0.2\" stroke=\"red\" fill=\"red\" />\n" % (x+0.5,y+0.5))
    ...
    for i in range(0,len(lista)):
        ...
        if palavra[0] == '1':
            escreveLinha(c, l, c+1, l,arquivo)
        if palavra[1] == '1':
            escreveLinha(c+1, l, c+1, l+1,arquivo)
        if palavra[2] == '1':
            escreveLinha(c, l+1, c+1, l+1,arquivo)
        if palavra[3] == '1':
            escreveLinha(c, l, c, l+1,arquivo)
...
def main():
    ...
    for i in range(0, maxi+1):
        ...
        j = str(i)
        if j in g.fila:
            escreveCirculo(cg, lg,fr,metragem)
